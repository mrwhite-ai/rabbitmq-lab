...
public class OrderConsumerOne {
    ...
    @RabbitListener(queues = RabbitMqConstants.ORDER_QUEUE, id = "order-consumer-1")
    public void consume(OrderCreatedEvent event) {
        log.info("[CONSUMER-1] Message received for order {}", event.getOrderId());
        orderProcessingService.processOrder("CONSUMER-1", event);
    }
}
